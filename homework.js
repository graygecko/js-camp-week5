...
function getAllCategories(products) {
  const categoryList = products.map(function (item) {
...
function addToCart(carts, product, quantity) {
  const existingCartItem = carts.find(function (item) {
...
function generateOrderReport(orders) {
  ...
  const averageOrderValue = totalOrders === 0 ? 0 : totalAmount / totalOrders;
...
function groupOrdersByPayment(orders) {
  return orders.reduce(function (result, order) {
    ...
    result[paymentMethod].push(order);
    return result;
  }, {});
}
